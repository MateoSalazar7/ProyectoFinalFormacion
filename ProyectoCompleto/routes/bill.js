...
router.post('/getPdf', auth.authenticateToken, function (req, res) {
    ...
    if (fs.existsSync(pdfPath)) {
        ...
        var productDetailsReport = JSON.parse(orderDetails.productDetails);
        ejs.renderFile(path.join(__dirname, '', "report.ejs"), { productDetails: productDetailsReport, name: orderDetails.name, email: orderDetails.email, contactNumber: orderDetails.contactNumber, paymentMethod: orderDetails.paymentMethod, totalAmount: orderDetails.totslAmount }, (err, results) => {
            if (err) {
                return res.status(500).json(err);
            }
            else {
